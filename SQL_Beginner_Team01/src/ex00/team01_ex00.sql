-- CTE1: Создаем временную таблицу с агрегированными данными по пользователям и балансу
WITH u_volume AS (
    SELECT
...
-- CTE2: Объединяем данные из CTE1 с информацией о валюте
b_cur_name AS (
    SELECT
        uv.name,
        uv.lastname,
...
-- CTE3: Получаем последнюю дату обновления для каждой валюты
rtu AS (
    SELECT
        currency.id,
        name,
...
-- Основной запрос: Объединяем данные из CTE2 с информацией о курсе валюты
SELECT
